"""
Model v5 for the CSE 251B NanoGPT competition.

Architecture target: stay under the 100M parameter cap while shifting capacity
from width into depth. Compared with model_final's 13L x 640d multi-head
attention profile, this default is a deeper/thinner 20L x 576d model with
3 KV heads for grouped-query attention.
...
Components:
  ...
  * SwiGLU MLP
  * Grouped-query attention (n_head query heads, n_kv_head KV heads)
  * Bias-free linears
  * Tied input/output embeddings
  * Optional Gemma-style logit softcap

Default config: 20 layers · 9 query heads · 3 KV heads · 576 embd · head_dim 64.
Expected parameter count: ~99.78M.
"""
...
class GPTConfigV5:
    ...
    n_layer: int = 20
    n_head: int = 9
    n_kv_head: int = 3          # set equal to n_head for standard MHA
    n_embd: int = 576           # head_dim = 576 / 9 = 64
    rope_base: float = 10000.0
    use_qk_norm: bool = True
    logit_softcap: float = 0.0
    # SwiGLU hidden = round_to_64(8/3 * n_embd). For n_embd=576 -> 1536.
    mlp_hidden: int = 1536
...
# ---------------------------------------------------------------------------
# Attention with RoPE + QK-Norm + GQA

class CausalSelfAttention(nn.Module):
    def __init__(self, config: GPTConfigV5):
        super().__init__()
        ...
        self.c_attn = nn.Linear(config.n_embd, config.n_embd + 2 * self.kv_dim, bias=False)
...
def _config_from_checkpoint(checkpoint) -> GPTConfigV5:
    raw_cfg = checkpoint.get("config") if isinstance(checkpoint, dict) else None
    if isinstance(raw_cfg, GPTConfigV5):
        return raw_cfg
    if raw_cfg is not None and hasattr(raw_cfg, "__dict__"):
        allowed = GPTConfigV5.__dataclass_fields__
        values = {k: v for k, v in vars(raw_cfg).items() if k in allowed}
        if values:
            return GPTConfigV5(**values)

    args = checkpoint.get("args", {}) if isinstance(checkpoint, dict) else {}
    return GPTConfigV5(
        n_layer=int(args.get("n_layer", GPTConfigV5.n_layer)),
        mlp_hidden=int(args.get("mlp_hidden", GPTConfigV5.mlp_hidden)),
    )
...
def load_model(checkpoint_path: str, device: str = "cuda") -> torch.nn.Module:
    """Load a train_v5 checkpoint for cse251b evaluate.py.
