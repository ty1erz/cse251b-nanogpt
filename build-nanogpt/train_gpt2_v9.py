    ...
    def __init__(self, source_specs, B, T, process_rank, num_processes, seed=2025):
        ...
        if missing:
            ...
            raise FileNotFoundError(
                "Missing tokenized sources for v8:\n"
                f"{msg}\n"
...
def main():
    ...
    model_config.exp_name = "alvin_v8"
    ...
    if master_process:
        ...
        print(
            f"KD: weight={kd_weight:.3f} top_k={kd_top_k} temperature={kd_temperature:.2f} "
            "(online top-k KD, no offline full-logit distillation)"
        )
    ...
    for local_step in range(train_steps):
        ...
        for micro_step in range(grad_accum_steps):
            ...
            with torch.autocast(device_type=device_type, dtype=amp_dtype):
                ...
                teacher_topk_logits, teacher_topk_indices = torch.topk(teacher_logits, k=kd_top_k, dim=-1)
                ...
                kd_loss = -(teacher_topk_probs * student_topk_log_probs).sum(dim=-1).mean() * (kd_temperature ** 2)
                loss = (1.0 - kd_weight) * hard_loss + kd_weight * kd_loss
            loss = loss / grad_accum_steps
            loss_accum += loss.detach()
            hard_loss_accum += hard_loss.detach()
            kd_loss_accum += kd_loss.detach()
            loss.backward()

        if ddp:
            dist.all_reduce(loss_accum, op=dist.ReduceOp.AVG)
            dist.all_reduce(hard_loss_accum, op=dist.ReduceOp.AVG)
            dist.all_reduce(kd_loss_accum, op=dist.ReduceOp.AVG)
        ...
        if master_process:
            print(
                ...
                f"kd: {kd_loss_accum.item() / grad_accum_steps:.6f} | lr {lr:.4e} | norm: {norm:.4f} | "
            ...
            with open(log_file, "a", encoding="utf-8") as f:
                f.write(
                    ...
                    f"kd {kd_loss_accum.item() / grad_accum_steps:.6f}\n"
                )
