...
class DataLoaderLite:
    def __init__(self, B, T, process_rank, num_processes, split, data_roots: Optional[tuple] = None):
        self.B = B
        self.T = T
        ...
        self.train_shard_probs = None
        if self.split == "train":
            valid_lengths = [max(0, n - (self.T + 1)) for n in self.shard_token_counts]
            total_valid = sum(valid_lengths)
            if total_valid <= 0:
                raise RuntimeError("train shards are too short for the configured sequence length")
    ...
    def next_batch(self):
        B, T = self.B, self.T
        if self.split == "train":
            shard_idx = int(self.train_rng.choice(len(self.shards), p=self.train_shard_probs))
            tokens = load_tokens(self.shards[shard_idx])
            max_start = len(tokens) - (T + 1)
            if max_start < 0:
                raise RuntimeError(f"shard {self.shards[shard_idx]} is too short for T={T}")
            starts = self.train_rng.integers(0, max_start + 1, size=B)
            x = torch.empty((B, T), dtype=torch.long)
            y = torch.empty((B, T), dtype=torch.long)
            for i, start in enumerate(starts):
                buf = tokens[start : start + T + 1]
...
total_batch_size = 65536 # smaller effective batch for more optimizer updates per token
...
train_loader = DataLoaderLite(B=B, T=T, process_rank=ddp_rank, num_processes=ddp_world_size, split="train", data_roots=model_config.data_roots_train)
...
max_lr = 4e-4
min_lr = 4e-5
max_steps = math.ceil(train_loader.total_tokens / total_batch_size)
warmup_ratio = 0.01
...
for step in range(max_steps):
    ...
    if master_process:
        ...
        should_save = (step > 0 and step % 1000 == 0) or last_step
        should_eval = (step > 0 and step % 500 == 0) or last_step
        if should_eval:
            export_prefix = "submission" if should_save else "eval"
            export_dir = export_model_bundle(raw_model, step, export_prefix)
            ...
            print(
                f"evaluate.py step {step} | ppl {eval_results['perplexity']:.4f} "
                f"| loss {eval_results['avg_loss_nats']:.6f} "
