...
DEFAULT_SELECTED_POOL = "/data/fengfei/cse251b-nanogpt/build-nanogpt/v7_pools/selected_pool_v7_latest.npz"
PUBLIC_VAL_BIN = "/data/fengfei/cse251b-nanogpt/val.bin"
TRAIN_BIN = "/data/fengfei/cse251b-nanogpt/build-nanogpt/prepared_mixture_gpt2_full/train.bin"
...
class SelectedWindowLoader:
    def __init__(self, pool_path, train_bin, B, T, process_rank, num_processes, seed=2025):
        self.B = B
        self.T = T
        self.process_rank = process_rank
        self.num_processes = num_processes
        self.tokens = np.memmap(train_bin, dtype=np.uint16, mode="r")
        pool = np.load(pool_path)
        self.start_offsets = pool["start_offset"].astype(np.int64)
        self.source_ids = pool["source_id"].astype(np.int64)
        self.total_windows = len(self.start_offsets)
        self.rng = np.random.default_rng(seed + process_rank)
        self.source_counts = {"fineweb_remaining": 0, "cosmopedia": 0}
        self.source_names = {0: "fineweb_remaining", 1: "cosmopedia"}
    ...
    def next_batch(self):
        idx = self.rng.integers(0, self.total_windows, size=self.B)
        x = torch.empty((self.B, self.T), dtype=torch.long)
        y = torch.empty((self.B, self.T), dtype=torch.long)
        for i, pool_idx in enumerate(idx):
            start = int(self.start_offsets[pool_idx])
            source_id = int(self.source_ids[pool_idx])
            ...
            self.source_counts[self.source_names[source_id]] += 1
        return x, y
...
def main():
    ...
    selected_pool_path = DEFAULT_SELECTED_POOL
    resume_step = 26250
    ...
    if master_process:
        ...
        print(f"selected pool: {selected_pool_path}")

    with open(os.path.join(submission_dir, "config.json"), "r", encoding="utf-8") as f:
        model_config = GPTConfig(**json.load(f))
    model_config.exp_name = "alvin_v7"
    ...
    train_loader = SelectedWindowLoader(
        pool_path=selected_pool_path,
        train_bin=TRAIN_BIN,
        B=B,
        T=T,
    ...
    amp_dtype = torch.bfloat16 if device_type == "cuda" and torch.cuda.is_bf16_supported() else (torch.float16 if device_type == "cuda" else torch.float32)

    if master_process:
        print(f"model params: {count_params(raw_model):,}")
        ...
        print(f"lambda_kd={lambda_kd:.4f} (disabled by default)")
    ...
    def run_official_eval(model_dir):
    ...
    for local_step in range(train_steps):
        ...
        for micro_step in range(grad_accum_steps):
            ...
            with torch.autocast(device_type=device_type, dtype=amp_dtype):
                logits = model(x)
                hard_loss = F.cross_entropy(logits.reshape(-1, logits.size(-1)), y.reshape(-1))
                loss = hard_loss
        ...
        if master_process:
            ...
            with open(log_file, "a") as f:
                f.write(f"{step} train {loss_accum.item():.6f} hard {hard_loss_accum.item() / grad_accum_steps:.6f}\n")

            if step > resume_step and step % 50 == 0:
                counts = train_loader.consume_source_counts()
                msg = " ".join(f"{k}={v}" for k, v in counts.items())
                print(f"selected_pool_sampled {msg}")
                with open(log_file, "a") as f:
                    f.write(f"{step} selected_pool_sampled {msg}\n")

            if step > 0 and (step % 50 == 0 or last_step):
                export_dir = export_submission_bundle(raw_model, step)
                eval_results, eval_elapsed = run_official_eval(export_dir)
                with open(log_file, "a") as f:
                    f.write(f"{step} export {export_dir}\n")
                    f.write(
                        f"{step} evaluate ppl {eval_results['perplexity']:.4f} "
                        f"loss {eval_results['avg_loss_nats']:.6f} tok {eval_results['total_tokens_evaluated']} sec {eval_elapsed:.2f}\n"
                    )
                print(
                    f"evaluate.py val.bin | ppl {eval_results['perplexity']:.4f} | "
                    f"loss {eval_results['avg_loss_nats']:.6f} | tokens {eval_results['total_tokens_evaluated']:,} | time {eval_elapsed:.1f}s"
                )
