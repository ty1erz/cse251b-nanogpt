...
DEFAULT_INIT_SUBMISSION_DIR = "/data/fengfei/cse251b-nanogpt/build-nanogpt/log/v12/submission_20260501_alvin_v12_40949"
START_STEP = 40949

# v15_kd fine-tunes the best v12 checkpoint with GPT-2 XL corrective KD.
# Keep the v12 data mix: v14's cosmopedia mix was worse on official val.
V15_SOURCE_SPECS = [
    {"name": "fineweb", "path": "/data/fengfei/cse251b-nanogpt/build-nanogpt/tokenized_sources/fineweb_full", "weight": 0.59},
    {"name": "wikipedia", "path": "/data/fengfei/cse251b-nanogpt-zzv-train/data/wikipedia", "weight": 0.17},
    {"name": "science", "path": "/data/fengfei/cse251b-nanogpt-zzv-train/data/science", "weight": 0.12},
    {"name": "books", "path": "/data/fengfei/cse251b-nanogpt-zzv-train/data/books", "weight": 0.12},
]
...
def configure_zzw_optimizers(model, device_type):
...
def init_fresh_model(model):
    ...
    submission_model_template.py does not define custom init, and PyTorch's
...
class MultiSourceWindowLoader:
    def __init__(self, source_specs, B, T, process_rank, num_processes, seed=2025, pin_memory=False):
        ...
        if missing:
            ...
            raise FileNotFoundError("Missing tokenized sources for v15:\n" f"{msg}")
    ...
    def _build_source_cycle(self):
        # The v15/v12 mix is close to 59/17/12/12; a 100-sample cycle
        # represents it exactly and keeps source sampling deterministic.
        cycle_len = 100
...
def run_in_memory_eval(model, data_path, device, amp_dtype, block_size=1024, batch_size=8):
    ...
    if n_chunks == 0:
        raise ValueError(f"Data too small for block_size={block_size}, batch_size={batch_size}")
...
def get_current_distill_weight(ft_step, max_weight):
    if ft_step >= KD_WARMUP_STEPS:
        return max_weight
    return max_weight * max(0.0, ft_step / max(1, KD_WARMUP_STEPS))


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--init_submission", type=str, default=DEFAULT_INIT_SUBMISSION_DIR, help="Submission checkpoint dir to fine-tune from")
    parser.add_argument("--resume", type=str, default=None, help="Path to a v15 train_ckpt_*.pt checkpoint")
    ...
    parser.add_argument("--teacher_model", type=str, default=TEACHER_MODEL_NAME, help="HF name or local path for the KD teacher")
    parser.add_argument("--kd_weight", type=float, default=KD_MAX_WEIGHT, help="Maximum corrective KD weight")
    parser.add_argument("--kd_top_k", type=int, default=KD_TOP_K, help="Number of teacher top logits to distill")
    parser.add_argument("--eval_batch_size", type=int, default=EVAL_BATCH_SIZE, help="Batch size for 50-step in-memory eval")
...
def main():
    ...
    stage_cfg = {"start_step": START_STEP, "sources": V15_SOURCE_SPECS}
    ...
    if args.resume:
        ...
        model, model_config, init_ckpt_path = load_submission_model(args.init_submission, device)
    if target_step <= start_step:
        raise ValueError(f"target_step must be > start_step ({start_step}), got {target_step}")
    model.to(device)
    model_config.exp_name = "alvin_v15_v12_muon_kd"
    raw_model = model
    ...
    muon_optim, adam_optim, muon_params, adam_decay, adam_nodecay = configure_zzw_optimizers(raw_model, device_type)
    if args.resume:
    ...
    teacher_dtype = amp_dtype if device_type == "cuda" else torch.float32
    ...
    if teacher.config.vocab_size != raw_model.config.vocab_size:
        raise ValueError(
            f"Teacher vocab size {teacher.config.vocab_size} does not match student {raw_model.config.vocab_size}"
        )
    ...
    if master_process:
        if args.resume:
            ...
            print(f"init: loaded submission checkpoint {init_ckpt_path}")
        print(f"start step: {start_step}")
        print(f"micro_batch: {B}")
        print(f"student params: {count_params(raw_model):,}")
        print(f"teacher: {args.teacher_model}, params={count_params(teacher):,}")
        ...
        print(
            "loss: hard CE + corrective top-k KD | "
            f"kd_weight={args.kd_weight:.3f} kd_warmup={KD_WARMUP_STEPS} "
            f"T={KD_TEMPERATURE:.2f} top_k={args.kd_top_k}"
        )
    ...
    log_dir = os.path.join("log", "v15_kd")
    ...
    with open(log_file, "a" if args.resume else "w", encoding="utf-8") as f:
        if args.resume:
            ...
            f.write(f"# initialized from {args.init_submission} at step {start_step}, target_step {target_step}\n")
    ...
    for step in range(start_step + 1, target_step + 1):
        ...
        gold_in_topk_accum = 0.0
        selected_fraction_accum = 0.0
        current_distill_weight = get_current_distill_weight(ft_step, args.kd_weight)
        for micro_step in range(grad_accum_steps):
            ...
            with torch.autocast(device_type=device_type, dtype=amp_dtype):
                ...
                teacher_topk_logits, teacher_topk_indices = torch.topk(teacher_logits, k=args.kd_top_k, dim=-1)
                teacher_topk_probs = F.softmax(teacher_topk_logits / KD_TEMPERATURE, dim=-1)
                student_topk_logits = torch.gather(student_logits, dim=-1, index=teacher_topk_indices)
                student_topk_log_probs = F.log_softmax(student_topk_logits / KD_TEMPERATURE, dim=-1)
                kd_per_pos = -(teacher_topk_probs * student_topk_log_probs).sum(dim=-1)

                gold_in_topk = (teacher_topk_indices == y.unsqueeze(-1)).any(dim=-1)
                selected_mask = gold_in_topk

                selected_count = selected_mask.sum()
                if selected_count.item() > 0:
                    distill_loss = kd_per_pos[selected_mask].mean() * (KD_TEMPERATURE ** 2)
                else:
                    distill_loss = hard_loss.new_zeros(())
                loss = (1.0 - current_distill_weight) * hard_loss + current_distill_weight * distill_loss
                gold_in_topk_frac = gold_in_topk.float().mean()
            ...
            gold_in_topk_accum += gold_in_topk_frac.detach()
            selected_fraction_accum += selected_fraction.detach()
            loss.backward()

        if ddp:
            dist.all_reduce(loss_accum, op=dist.ReduceOp.AVG)
            dist.all_reduce(hard_loss_accum, op=dist.ReduceOp.AVG)
            dist.all_reduce(distill_loss_accum, op=dist.ReduceOp.AVG)
            dist.all_reduce(gold_in_topk_accum, op=dist.ReduceOp.AVG)
            dist.all_reduce(selected_fraction_accum, op=dist.ReduceOp.AVG)

        norm = torch.nn.utils.clip_grad_norm_(raw_model.parameters(), GRAD_CLIP)
        muon_lr_now = get_lr(ft_step, MUON_LR, muon_min_lr)
        adam_lr_now = get_lr(ft_step, ADAM_LR, adam_min_lr)
        ...
        eval_due = step > start_step and ((step - start_step) % eval_interval == 0 or last_step)
        save_due = step > start_step and ((step - start_step) % save_interval == 0 or last_step)

        if master_process:
            print(
                f"step {step:5d} | loss: {loss_accum.item():.6f} | "
                f"hard: {hard_loss_accum.item() / grad_accum_steps:.6f} | "
                f"distill: {distill_loss_accum.item() / grad_accum_steps:.6f} | "
                f"kd_w: {current_distill_weight:.4f} | "
                f"gold@topk: {gold_in_topk_accum.item() / grad_accum_steps:.4f} | "
                f"selected: {selected_fraction_accum.item() / grad_accum_steps:.4f} | "
            ...
            with open(log_file, "a", encoding="utf-8") as f:
                f.write(
                    ...
                    f"distill {distill_loss_accum.item() / grad_accum_steps:.6f} "
                    f"kd_w {current_distill_weight:.4f} "
                    f"gold_topk_frac {gold_in_topk_accum.item() / grad_accum_steps:.6f} "
                    f"selected_frac {selected_fraction_accum.item() / grad_accum_steps:.6f} "
            ...
            if save_due:
                ...
                t_eval = time.time()
                eval_results = run_in_memory_eval(
                    raw_model,
                    DEFAULT_PUBLIC_VAL,
                    device,
                    amp_dtype,
                ...
                with open(log_file, "a", encoding="utf-8") as f:
                    f.write(
                        f"{step} quick_eval ppl {eval_results['perplexity']:.4f} "
                        f"loss {eval_results['avg_loss_nats']:.6f} "
                        f"tok {eval_results['total_tokens_evaluated']} sec {eval_elapsed:.2f}\n"
                    )
                print(
                    f"quick val.bin | ppl {eval_results['perplexity']:.4f} | "
                    f"loss {eval_results['avg_loss_nats']:.6f} | tokens {eval_results['total_tokens_evaluated']:,} | "
                    f"time {eval_elapsed:.1f}s"
                )

        if ddp and (eval_due or save_due):
            dist.barrier()
