...
# create model
resume_checkpoint = "/data/fengfei/cse251b-nanogpt/build-nanogpt/log/v4/submission_20260426_alvin_v4_26000/checkpoint.pt"
resume_step = 26000
...
train_loader = DataLoaderLite(
    ...
    dataset_ranges=dataset_ranges_v5,
)
...
if master_process:
    ...
    print(f"remaining total tokens for v5 distill: {remaining_total_tokens:,}")
    print(f"max_steps for one pass over full train.bin: {max_steps:,}")
    print(f"remaining v5 distill steps: {remaining_steps:,}")
    print(f"teacher model: {teacher_model_name}")
    print(
        f"hard_weight=1-kd_w kd_max_weight={kd_max_weight:.2f} "
    ...
    print("v5 distill dataset ranges:")
    for r in dataset_ranges_v5:
...
log_dir = os.path.join("log", "v5_distill")
...
for local_step in range(remaining_steps):
    ...
    for micro_step in range(grad_accum_steps):
        ...
        with torch.autocast(device_type=device_type, dtype=amp_dtype):
            ...
            selected_fraction = gold_in_topk.float().mean()
            selected_count = gold_in_topk.sum()
            if selected_count.item() > 0:
                distill_loss = kd_per_pos[gold_in_topk].mean() * (distill_temperature ** 2)
            else:
    ...
    if master_process:
        print(
            ...
            f"gold@topk: {selected_fraction_accum.item() / grad_accum_steps:.4f} | "
        ...
        with open(log_file, "a") as f:
            f.write(
                ...
                f"distill {distill_loss_accum.item() / grad_accum_steps:.6f} "
                f"kd_w {current_distill_weight:.4f} "
                f"gold_topk_frac {selected_fraction_accum.item() / grad_accum_steps:.6f}\n"
            )
