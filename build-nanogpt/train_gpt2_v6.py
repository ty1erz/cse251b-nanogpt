...
class HiddenDistillHeads(nn.Module):
    def __init__(self, student_hidden_size, teacher_hidden_size, student_layers):
        super().__init__()
        self.projections = nn.ModuleDict(
            {
                str(layer): nn.Linear(student_hidden_size, teacher_hidden_size, bias=False)
                for layer in student_layers
            }
        )

    def forward(self, layer_idx, hidden_states):
        return self.projections[str(layer_idx)](hidden_states)


def configure_joint_adamw(modules, weight_decay, learning_rate, device_type, beta1=0.9, beta2=0.95):
    param_dict = {}
    for module_name, module in modules.items():
        for pn, p in module.named_parameters():
            if p.requires_grad:
                param_dict[f"{module_name}.{pn}"] = p
    ...
    if master_process:
        num_decay_params = sum(p.numel() for p in decay_params)
        num_nodecay_params = sum(p.numel() for p in nodecay_params)
        print(f"joint AdamW decayed tensors: {len(decay_params)}, params: {num_decay_params:,}")
        print(f"joint AdamW non-decayed tensors: {len(nodecay_params)}, params: {num_nodecay_params:,}")
        print(f"using fused AdamW: {use_fused}")
...
class DataLoaderLite:
    def __init__(
        ...
        self.train_rng = np.random.default_rng(2026 + process_rank)
        self.source_counts = {}
...
# create model
resume_checkpoint = "/data/fengfei/cse251b-nanogpt/build-nanogpt/log/v5_distill/submission_20260427_alvin_v5_distill_26250/checkpoint.pt"
resume_step = 26250
...
train_loader = DataLoaderLite(
    ...
    dataset_ranges=dataset_ranges_v6,
)
...
short_run_steps = 500
remaining_steps = min(short_run_steps, math.ceil(remaining_total_tokens / total_batch_size))
warmup_steps = 50
teacher_model_name = "gpt2-xl"
teacher_hidden_size = 1600
student_hidden_size = model_config.n_embd
teacher_student_layer_pairs = [(4, 12), (8, 24), (12, 36), (16, 48)]
student_distill_layers = [s for s, _ in teacher_student_layer_pairs]
hidden_distill_positions = 128
lambda_hidden = 0.05
lambda_kd = 0.0
...
if master_process:
    ...
    print(f"remaining total tokens for v6: {remaining_total_tokens:,}")
    print(f"max_steps for one pass over full train.bin: {full_run_max_steps:,}")
    print(f"planned short v6 run steps: {remaining_steps:,}")
    print(f"teacher model: {teacher_model_name}")
    print(f"lr schedule: max_lr={max_lr:.2e} min_lr={min_lr:.2e} warmup_steps={warmup_steps}")
    print(
        f"hidden_distill: lambda_hidden={lambda_hidden:.4f} "
        f"positions_per_seq={hidden_distill_positions} lambda_kd={lambda_kd:.4f}"
    )
    print(f"teacher/student layer mapping: {teacher_student_layer_pairs}")
    print("v6 dataset ranges:")
    for r in dataset_ranges_v6:
...
def get_lr(it):
    ...
    if it > remaining_steps:
        return min_lr
    # 3) in between, use cosine decay down to min learning rate
    decay_ratio = (it - warmup_steps) / max(1, (remaining_steps - warmup_steps))
...
hidden_distill_heads = HiddenDistillHeads(
    student_hidden_size=student_hidden_size,
    teacher_hidden_size=teacher_hidden_size,
    student_layers=student_distill_layers,
)
hidden_distill_heads.to(device)

# optimize student + training-only projection heads
optimizer = configure_joint_adamw(
    modules={"student": raw_model, "hidden_heads": hidden_distill_heads},
    weight_decay=0.1,
...
if master_process:
    ...
    print(f"teacher hidden size: {teacher.config.n_embd}")
    if teacher.config.vocab_size != model_config.vocab_size:
        raise RuntimeError(
            f"Teacher vocab size {teacher.config.vocab_size} does not match student {model_config.vocab_size}"
        )
    if teacher.config.n_embd != teacher_hidden_size:
        raise RuntimeError(
            f"Teacher hidden size {teacher.config.n_embd} does not match expected {teacher_hidden_size}"
        )

# create the log directory we will write checkpoints to and log to
log_dir = os.path.join("log", "v6")
...
def export_training_aux(step):
    aux_dir = os.path.join(log_dir, f"train_aux_{run_date}_{model_config.exp_name}_{step:05d}")
    os.makedirs(aux_dir, exist_ok=True)
    aux_state = {
        "hidden_distill_heads": hidden_distill_heads.state_dict(),
        "teacher_student_layer_pairs": teacher_student_layer_pairs,
        "student_hidden_size": student_hidden_size,
        "teacher_hidden_size": teacher_hidden_size,
        "hidden_distill_positions": hidden_distill_positions,
        "lambda_hidden": lambda_hidden,
        "resume_checkpoint": resume_checkpoint,
        "resume_step": resume_step,
        "step": step,
    }
    torch.save(aux_state, os.path.join(aux_dir, "hidden_distill_aux.pt"))
    return aux_dir
...
def gather_hidden_positions(hidden_states, positions):
    gather_index = positions.unsqueeze(-1).expand(-1, -1, hidden_states.size(-1))
...
for local_step in range(remaining_steps):
    ...
    for micro_step in range(grad_accum_steps):
        ...
        with torch.no_grad():
            with torch.autocast(device_type=device_type, dtype=amp_dtype):
                teacher_out = teacher(x, output_hidden_states=True)
                teacher_hidden_states = teacher_out.hidden_states
        with torch.autocast(device_type=device_type, dtype=amp_dtype):
            student_logits, hard_loss, student_hidden_states = model(x, y, output_hidden_states=True)
            num_positions = min(hidden_distill_positions, x.size(1))
            positions = torch.randint(0, x.size(1), (x.size(0), num_positions), device=x.device)
            per_layer_hidden_losses = []
            for student_layer, teacher_layer in teacher_student_layer_pairs:
                student_hidden = student_hidden_states[student_layer - 1]
                teacher_hidden = teacher_hidden_states[teacher_layer]
                student_hidden = gather_hidden_positions(student_hidden, positions)
                teacher_hidden = gather_hidden_positions(teacher_hidden, positions)
                projected_student = hidden_distill_heads(student_layer, student_hidden)
                projected_student = F.normalize(projected_student.float(), dim=-1)
                teacher_hidden = F.normalize(teacher_hidden.float(), dim=-1)
                layer_hidden_loss = (1.0 - (projected_student * teacher_hidden).sum(dim=-1)).mean()
                per_layer_hidden_losses.append(layer_hidden_loss)
            hidden_loss = torch.stack(per_layer_hidden_losses).mean()
            total_loss = hard_loss + lambda_hidden * hidden_loss
            loss = total_loss
        ...
        if micro_step == 0:
            ...
            hidden_loss_accum = hidden_loss.detach()
        else:
            hard_loss_accum += hard_loss.detach()
            hidden_loss_accum += hidden_loss.detach()
        loss.backward()
    if ddp:
        ...
        dist.all_reduce(hidden_loss_accum, op=dist.ReduceOp.AVG)
    norm = torch.nn.utils.clip_grad_norm_(
        list(raw_model.parameters()) + list(hidden_distill_heads.parameters()),
        1.0,
    )
    ...
    if master_process:
        print(
            ...
            f"hidden: {hidden_loss_accum.item() / grad_accum_steps:.6f} | "
            f"lambda_hidden: {lambda_hidden:.4f} | "
            f"lr {lr:.4e} | norm: {norm:.4f} | dt: {dt*1000:.2f}ms | tok/sec: {tokens_per_sec:.2f}"
        )
        with open(log_file, "a") as f:
            f.write(
