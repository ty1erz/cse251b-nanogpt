...
# v14 keeps the v13 architecture/optimizer and adds Cosmopedia to the mix.
V14_SOURCE_SPECS = [
    {"name": "fineweb", "path": "/data/fengfei/cse251b-nanogpt/build-nanogpt/tokenized_sources/fineweb_full", "weight": 0.50},
    {"name": "cosmopedia", "path": "/data/fengfei/cse251b-nanogpt/build-nanogpt/tokenized_sources/cosmopedia_full", "weight": 0.15},
    {"name": "science", "path": "/data/fengfei/cse251b-nanogpt-zzv-train/data/science", "weight": 0.15},
    {"name": "wikipedia", "path": "/data/fengfei/cse251b-nanogpt-zzv-train/data/wikipedia", "weight": 0.10},
    {"name": "books", "path": "/data/fengfei/cse251b-nanogpt-zzv-train/data/books", "weight": 0.10},
]
...
class MultiSourceWindowLoader:
    def __init__(self, source_specs, B, T, process_rank, num_processes, seed=2025, pin_memory=False):
        ...
        if missing:
            ...
            raise FileNotFoundError("Missing tokenized sources for v14:\n" f"{msg}")
    ...
    def _build_source_cycle(self):
        # The v14 mix is 50/15/15/10/10, so a 20-sample cycle is exact.
...
def parse_args():
    ...
    parser.add_argument("--resume", type=str, default=None, help="Path to a v14 train_ckpt_*.pt checkpoint")
...
def main():
    ...
    stage_cfg = {"start_step": START_STEP, "sources": V14_SOURCE_SPECS}
    ...
    model.to(device)
    model_config.exp_name = "alvin_v14_cosmo_mix_muon"
    raw_model = model
