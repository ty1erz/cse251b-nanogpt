...
train_loader = DataLoaderLite(
    ...
    dataset_ranges=dataset_ranges_v7,
)
...
if master_process:
    ...
    print(f"remaining total tokens for v7: {remaining_total_tokens:,}")
    print(f"short v7 run steps: {remaining_steps:,}")
    print(f"teacher model: {teacher_model_name}")
    print(
        f"teacher-corrective KD | kd_max_weight={kd_max_weight:.2f} "
        f"T={distill_temperature:.2f} top_k={distill_top_k}"
    )
    print(f"lr schedule: max_lr={max_lr:.2e} min_lr={min_lr:.2e} warmup_steps={warmup_steps}")
    print(f"kd warmup: linear 0->{kd_max_weight:.2f} over first {kd_warmup_steps} steps")
    print("v7 dataset ranges:")
    for r in dataset_ranges_v7:
...
def get_current_distill_weight(local_step):
    if local_step >= kd_warmup_steps:
        return kd_max_weight
    progress = max(0.0, local_step / max(1, kd_warmup_steps))
    return kd_max_weight * progress
...
for local_step in range(remaining_steps):
    ...
    optimizer.zero_grad()
    loss_accum = 0.0
    current_distill_weight = get_current_distill_weight(local_step)
    for micro_step in range(grad_accum_steps):
        ...
        with torch.autocast(device_type=device_type, dtype=amp_dtype):
            student_logits, hard_loss = model(x, y)
            teacher_topk_logits, teacher_topk_indices = torch.topk(
                teacher_logits, k=distill_top_k, dim=-1
            )
            teacher_topk_probs = F.softmax(teacher_topk_logits / distill_temperature, dim=-1)
            student_topk_logits = torch.gather(
                student_logits, dim=-1, index=teacher_topk_indices
            )
            student_topk_log_probs = F.log_softmax(student_topk_logits / distill_temperature, dim=-1)
            kd_per_pos = -(teacher_topk_probs * student_topk_log_probs).sum(dim=-1)
            gold_in_topk = (teacher_topk_indices == y.unsqueeze(-1)).any(dim=-1)

            teacher_log_probs_full = F.log_softmax(teacher_logits / distill_temperature, dim=-1)
            student_log_probs_full = F.log_softmax(student_logits / distill_temperature, dim=-1)
            teacher_gold_logp = torch.gather(
                teacher_log_probs_full, dim=-1, index=y.unsqueeze(-1)
            ).squeeze(-1)
            student_gold_logp = torch.gather(
                student_log_probs_full, dim=-1, index=y.unsqueeze(-1)
            ).squeeze(-1)
            teacher_better = teacher_gold_logp > student_gold_logp
            selected_mask = gold_in_topk & teacher_better

            gold_in_topk_frac = gold_in_topk.float().mean()
            teacher_better_frac = teacher_better.float().mean()
            selected_fraction = selected_mask.float().mean()
            selected_count = selected_mask.sum()
            if selected_count.item() > 0:
                distill_loss = kd_per_pos[selected_mask].mean() * (distill_temperature ** 2)
            else:
                distill_loss = hard_loss.new_zeros(())
            loss = hard_loss + current_distill_weight * distill_loss
        ...
        if micro_step == 0:
            ...
            teacher_better_accum = teacher_better_frac.detach()
    ...
    if ddp:
        ...
        dist.all_reduce(teacher_better_accum, op=dist.ReduceOp.AVG)
    ...
    if master_process:
        print(
            ...
            f"teacher_better: {teacher_better_accum.item() / grad_accum_steps:.4f} | "
        ...
        with open(log_file, "a") as f:
            f.write(
                ...
                f"teacher_better_frac {teacher_better_accum.item() / grad_accum_steps:.6f} "
