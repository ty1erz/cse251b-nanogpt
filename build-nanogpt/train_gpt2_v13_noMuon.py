...
# v13_noMuon keeps v13's architecture/data/batch/schedule, but removes Muon.
V13_SOURCE_SPECS = [
...
def split_params_like_v13(model):
    hidden_decay, embed_nodecay, nodecay = [], [], []
    seen = set()
    for name, p in model.named_parameters():
        ...
        if p.ndim < 2:
            ...
            embed_nodecay.append(p)
        else:
            hidden_decay.append(p)
    return hidden_decay, embed_nodecay, nodecay


def configure_adamw_optimizer(model, device_type):
    hidden_decay, embed_nodecay, nodecay = split_params_like_v13(model)
    optimizer = torch.optim.AdamW(
        [
            {"params": hidden_decay, "weight_decay": WEIGHT_DECAY},
    ...
    return optimizer, hidden_decay, embed_nodecay, nodecay
...
class MultiSourceWindowLoader:
    def __init__(self, source_specs, B, T, process_rank, num_processes, seed=2025, pin_memory=False):
        ...
        if missing:
            ...
            raise FileNotFoundError("Missing tokenized sources for v13:\n" f"{msg}")
    ...
    def _build_source_cycle(self):
        # The v13 mix is 50/20/15/15, so a 20-sample cycle is exact.
...
def parse_args():
    ...
    parser.add_argument("--resume", type=str, default=None, help="Path to a v13_noMuon train_ckpt_*.pt checkpoint")
...
def main():
    ...
    stage_cfg = {"start_step": START_STEP, "sources": V13_SOURCE_SPECS}
    ...
    model.to(device)
    model_config.exp_name = "alvin_v13_zzw_mix_noMuon"
    raw_model = model
    ...
    optimizer, hidden_decay, embed_nodecay, adam_nodecay = configure_adamw_optimizer(raw_model, device_type)
    if args.resume:
    ...
    if master_process:
        ...
        print(
            "AdamW -> "
            f"{len(hidden_decay) + len(embed_nodecay) + len(adam_nodecay)} tensors, "
            f"{sum(p.numel() for p in hidden_decay) + sum(p.numel() for p in embed_nodecay) + sum(p.numel() for p in adam_nodecay):,} params, "
            "all trainable params"
        )
        print(
            "AdamW groups: "
            f"hidden_decay={sum(p.numel() for p in hidden_decay):,} "
            f"embed_nodecay={sum(p.numel() for p in embed_nodecay):,} "
            f"nodecay={sum(p.numel() for p in adam_nodecay):,}"
        )
        print("source weights:")
        for s in train_loader.sources:
            print(f"  {s['name']}: weight={s['weight']:.2f} path={s['path']}")
        print(
            f"training: optimizer=AdamW-only adam_lr={ADAM_LR:.2e} min_lr_ratio={MIN_LR_RATIO} "
    ...
    log_dir = os.path.join("log", "v13_noMuon")
    ...
    for step in range(start_step + 1, target_step + 1):
        ...
        if master_process:
            print(
                ...
                f"adam {adam_lr_now:.4e} | norm: {norm:.4f} | "
