...
DEFAULT_RESUME_SUBMISSION_DIR = "/data/fengfei/cse251b-nanogpt/build-nanogpt/log/v8/submission_20260428_alvin_v8_30950"
DEFAULT_RESUME_STEP = 30950
...
class MultiSourceWindowLoader:
    def __init__(self, source_specs, B, T, process_rank, num_processes, seed=2025):
        ...
        for spec in source_specs:
            ...
            source = {"name": spec["name"], "path": path, "weight": float(spec["weight"])}
            if os.path.isdir(path) and os.path.isfile(os.path.join(path, "train.bin")):
                ...
                source.update({"mode": "train_bin", "tokens": tokens, "start": start, "end": end})
                ...
                source.update({"mode": "npy_shards", "shards": shard_tokens, "shard_paths": shard_paths})
            else:
        ...
        if missing:
            ...
            raise FileNotFoundError("Missing tokenized sources for v12:\n" f"{msg}")
    ...
    def next_batch(self):
        ...
        for i, source_idx in enumerate(choices):
            ...
            if source["mode"] == "train_bin":
                ...
                shard = source["shards"][int(self.rng.integers(0, len(source["shards"])))]
                high = len(shard) - (self.T + 1)
                start = int(self.rng.integers(0, high + 1))
                buf = np.asarray(shard[start : start + self.T + 1], dtype=np.int64)
...
def main():
    ...
    stage_cfg = {"resume_submission_dir": DEFAULT_RESUME_SUBMISSION_DIR, "resume_step": DEFAULT_RESUME_STEP, "sources": V12_SOURCE_SPECS}
    ...
    model_config.exp_name = "alvin_v12"
