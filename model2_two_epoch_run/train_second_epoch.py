#!/usr/bin/env python3
"""
Second-epoch continuation training.

This is intentionally plain CE training: same model/data/loader style as the
first epoch, with a low-LR resume schedule for the second epoch.
"""
...
MICRO_BATCH      = 16
SEQ_LEN          = 1024
TOTAL_BATCH_SIZE = 524288          # 512 K tokens / step
TARGET_STEP      = 76000           # continue step 38000 toward a second epoch
...
def parse_args():
    ...
    p.add_argument("--resume", default=None, help="Path to the first-epoch step-38000 checkpoint or later")
...
def main():
    ...
    train_loader = MultiSourceWindowLoader(
        source_specs=SECOND_EPOCH_SOURCE_SPECS, B=B, T=T,
    ...
    if args.resume is None:
        raise ValueError(
            "Second-epoch training requires --resume "
            "log/first_epoch/train_ckpt_38000.pt"
        )

    # Load the first-epoch checkpoint.
    ...
    model.to(device)
    model_config.exp_name = args.run_name or "second_epoch"
    raw_model = model
    ...
    if master_process:
        print(f"resumed first-epoch checkpoint from {args.resume} at step {start_step}")
    ...
    log_dir = os.path.join(LOG_ROOT, "second_epoch")
    ...
    with open(log_file, "a" if args.resume else "w") as f:
        f.write(
            f"# second-epoch continuation from {args.resume} at step {start_step}, "
    ...
    def export_submission_bundle(model_obj, step):
        """Export raw second-epoch model weights."""
