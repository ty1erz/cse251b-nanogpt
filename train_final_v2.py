"""
Final trainer v2 — mix experiment on top of v1 (SwiGLU + tuned LR).

  Only difference vs train_final_v1.py:
    * Mix shifted slightly fineweb-heavy: 59 / 17 / 12 / 12
      (was 50 / 20 / 15 / 15 — TA's stated example eval blend)
    * log_root: log_final_v2/ (separate from v1's outputs)

  Same as train_final_v1.py:
    * model_final.GPT — 13 layers, SwiGLU MLP, n_embd=640, mlp_hidden=1728
    * ~96.6 M params
    * Muon + AdamW split optimizer, peak muon=1.3e-2 / adam=5.2e-4
    * 38000 steps cosine to 0.1× peak, warmup=200, ckpt every 2000
    * Full seamless resume (model + optims + loader state + RNG)

  Caveat: deviating from TA's example 50/20/15/15 is an experiment, not a
  proven win. The README says val.bin is drawn from the same distribution as
  the hidden test set, which is multi-domain. If the actual test blend really
  is close to 50/20/15/15, this run may score WORSE than v1 (19.4353 PPL).
  Compare carefully.

Launch (fresh):
    python train_final_v2.py --run_name final_v2_mix5917

Launch (resume):
    python train_final_v2.py --run_name final_v2_mix5917 \
        --resume log_final_v2/final_v2_mix5917/model_010000.pt
"""
...
DEFAULTS = dict(
    run_name=None,
    log_root=os.path.join(HERE, "log_final_v2"),
    resume=None,
    # data
    total_batch_size=524288,
    micro_batch=16,
    seq_len=1024,
    # schedule — one full epoch over ~20 B mixed corpus
    max_steps=38000,
    warmup_steps=200,
    ckpt_every=2000,
    eval_every=250,
    eval_iters=20,
    # optim — same peak LRs as v1
    muon_lr=1.3e-2,
    muon_momentum=0.95,
    adam_lr=5.2e-4,
    min_lr_ratio=0.1,
...
- Seed: {args.seed}
- Run script: `train_final_v2.py --run_name {args.run_name} --max_steps {args.max_steps}`
...
def main():
    ...
    if args.run_name is None:
        args.run_name = f"final_v2_mix5917_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}"
    ...
    train_loader = MixedDataLoader(
        ...
        split="train", master_process=master, mix=FINAL_V2_MIX,
    )
    val_loader = MixedDataLoader(
        B=B, T=T, process_rank=ddp_rank, num_processes=ddp_world_size,
        split="val", master_process=master, mix=FINAL_V2_MIX,
    )
