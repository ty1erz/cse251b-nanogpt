"""
Model 1 ratio ablation: 50/20/15/15 mix for one 38k-step epoch.
...
Optimizer: Muon for hidden matrices and AdamW for embeddings/norm parameters.

See README.md for the exact main-run and continuation commands.
"""
...
from model import GPT, GPTConfigFinal, num_params  # noqa: E402
...
- Seed: {args.seed}
- Run script: `train_model1_mix50.py --run_name {args.run_name} --max_steps {args.max_steps}`
...
def main():
    ...
    if args.run_name is None:
        args.run_name = f"model1_mix50_main_38k_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}"
