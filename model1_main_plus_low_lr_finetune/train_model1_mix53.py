"""
Model 1 ratio ablation: 53/19/14/14 mix for one 38k-step epoch.

Architecture: 13 layers, 640 hidden size, 10 heads, SwiGLU hidden size 1728,
RoPE, RMSNorm, QK-Norm, and 96.64M parameters.
Data mix: FineWeb/Wikipedia/science/books = 53/19/14/14.
...
- Seed: {args.seed}
- Run script: `train_model1_mix53.py --run_name {args.run_name} --max_steps {args.max_steps}`
...
def main():
    ...
    if args.run_name is None:
        args.run_name = f"model1_mix53_main_38k_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}"
